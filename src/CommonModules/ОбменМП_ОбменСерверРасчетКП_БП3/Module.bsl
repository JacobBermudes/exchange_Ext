...
Функция ПолучитьВыборкуНачислений(ВыборкаРазрешенныхЗданийПомесячно, ПараметрыВыгрузки, НастройкиОбмена) экспорт
	...
	//Преобразование таблицы с идентификаторами в таблицу значений со ссылками
	...
	ТаблицаРазрешенныхЗданий.Колонки.Добавить("ВыгружатьНачисления", Новый ОписаниеТипов("Булево"));
	...
	Пока ВыборкаРазрешенныхЗданийПомесячно.Следующий() Цикл
		Если ТипЗнч(ВыборкаРазрешенныхЗданийПомесячно.УникальныйИдентификатор) = Тип("УникальныйИдентификатор") Тогда
			...
			НоваяСтрока.ВыгружатьНачисления = ВыборкаРазрешенныхЗданийПомесячно.ВыгружатьНачисления;
		КонецЕсли;
	КонецЦикла;
	
	СписокВидовРасчетаНевыводить = Новый СписокЗначений;
	СписокВидовРасчетаНевыводить.Добавить(Справочники.ркВидыРасчетов.НайтиПоКоду("000000030"));
	СписокВидовРасчетаНевыводить.Добавить(Справочники.ркВидыРасчетов.НайтиПоКоду("000009644"));
	...
	|	ТаблицаРазрешенныхЗданий.ВыгружатьНачисления
	...
	|	ркЗадолженностьЛицевыхСчетовОстаткиИОбороты.ВидРасчета КАК ВидНачисления,
	|	ЕдиницыИзмерений.ЕдиницаРасчетаЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	СУММА(ЕСТЬNULL(ркЗадолженностьЛицевыхСчетовОстаткиИОбороты.СуммаНачальныйОстаток, 0)) КАК НачальныйОстаток,
	|	СУММА(ЕСТЬNULL(ркНачисленияОбороты.КоличествоВПределахНормыОборот + ркНачисленияОбороты.КоличествоСверхНормыОборот,
	|		0)) КАК Количество,
	|	СУММА(ЕСТЬNULL(ркЗадолженностьЛицевыхСчетовОстаткиИОбороты.СуммаРасход, 0)) КАК Оплачено,
	|	СУММА(ВЫБОР
	|		КОГДА ЛицевыеСчета.ВыгружатьНачисления
	|			ТОГДА ЕСТЬNULL(ркНачисленияОбороты.СуммаНачисленийОборот, 0)
	|		ИНАЧЕ 0
	|	КОНЕЦ) КАК Начислено,
	|	СУММА(ВЫБОР
	|		КОГДА ЛицевыеСчета.ВыгружатьНачисления
	|			ТОГДА ркЗадолженностьЛицевыхСчетовОстаткиИОбороты.СуммаКонечныйОстаток
	|		ИНАЧЕ ркЗадолженностьЛицевыхСчетовОстаткиИОбороты.СуммаКонечныйОстаток -
	|			ЕСТЬNULL(ркНачисленияОбороты.СуммаНачисленийОборот, 0)
	...
	|			Месяц,, ВидРасчета НЕ В (&СписокВидовРасчетаНевыводить)) КАК ркЗадолженностьЛицевыхСчетовОстаткиИОбороты
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ркНачисления.Обороты(&НачалоПериода, &КонецПериода, Месяц,) КАК
	|				ркНачисленияОбороты
	...
	Запрос.УстановитьПараметр("СписокВидовРасчетаНевыводить", СписокВидовРасчетаНевыводить);
...
// Параметры:
//	ТаблицаОплат - ТаблицаЗначений
Процедура СоздатьИЗаполнитьДокументыОплат(ТаблицаОплат) экспорт
	
	СпособОплаты = Справочники.ркСпособыОплат.НайтиПоНаименованию("Мобильное приложение");
	Если СпособОплаты.Пустая() тогда
		СпособОплаты = Справочники.ркСпособыОплат.СоздатьЭлемент(); 
		СпособОплаты.Наименование = "Мобильное приложение";
		СпособОплаты.Записать();
	КонецЕсли;
	
	ПулДокументов = Новый ТаблицаЗначений;
	ПулДокументов.Колонки.Добавить("ДокументОбъект");
	ПулДокументов.Колонки.Добавить("Дата");
	
	Для каждого ЗагруженнаяОплата из ТаблицаОплат цикл
		
		Документ = ПулДокументов.Найти(ЗагруженнаяОплата.ДатаПлатежа, "Дата");
		Если Документ = Неопределено тогда
			Документ = Документы.ркОплата.СоздатьДокумент();
			Документ.Дата = ЗагруженнаяОплата.ДатаПлатежа;
			...
			Документ.ОтложенноеРаспределение = Ложь;
			
			СтрокаДокументаПул = ПулДокументов.Добавить();
			СтрокаДокументаПул.ДокументОбъект = Документ;
			СтрокаДокументаПул.Дата = ЗагруженнаяОплата.ДатаПлатежа;
		ИначеЕсли ТипЗнч(Документ) = Тип("СтрокаТаблицыЗначений") тогда
			Документ = Документ.ДокументОбъект;
		КонецЕсли;
			
		ЛицевойСчет = Справочники.ркЛицевыеСчета.НайтиПоКоду(ЗагруженнаяОплата.НомерСчета);
		Если ЛицевойСчет.Пустая() тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаОплаты = Документ.Состав.Добавить();
		СтрокаОплаты.ЛицевойСчет = ЛицевойСчет;
		СтрокаОплаты.ДатаОплаты = ЗагруженнаяОплата.ДатаПлатежа;
		СтрокаОплаты.Сумма = ЗагруженнаяОплата.Сумма;
		//Запишем объект, при обходе пула для записи документов произведём замену на ссылку
		ЗагруженнаяОплата.ДокументОплаты = Документ;
		
	КонецЦикла;

	Для каждого ДокументИзПула из ПулДокументов цикл
		
		Документы.ркОплата.ВыполнитьРаспределениеОплаты(ДокументИзПула.ДокументОбъект);
		ДокументИзПула.ДокументОбъект.Записать(РежимЗаписиДокумента.Проведение);
		
		//Замена ДокОбект на уникальный идентификатор для записи в регистр ID оплат
		СтрокиСДокументом = ТаблицаОплат.НайтиСтроки(Новый Структура("ДокументОплаты", ДокументИзПула.ДокументОбъект));
		Для каждого СтрокаДляРегистраIDПлатежей из СтрокиСДокументом цикл
			УникальныйИдентификаторОплаты = ДокументИзПула.ДокументОбъект.Ссылка.УникальныйИдентификатор();
			СтрокаДляРегистраIDПлатежей.ДокументОплаты = УникальныйИдентификаторОплаты;
		КонецЦикла;
	КонецЦикла;
