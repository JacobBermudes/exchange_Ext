...
Процедура ВыгрузитьНачисления() Экспорт //++
	...
	Пока УзлыВыборка.Следующий() Цикл
		...
		//Сформируем помесячную таблицу зданий для выгрузки
		ПомесячнаяТаблицаЗданий = Новый ТаблицаЗначений;
		ПомесячнаяТаблицаЗданий.Колонки.Добавить("МесяцНачислений", Новый ОписаниеТипов("Дата"));
		ПомесячнаяТаблицаЗданий.Колонки.Добавить("УникальныйИдентификатор", Новый ОписаниеТипов("УникальныйИдентификатор"));
		
		//Необработанная таблица зданий и месяцев для выгрузки
		Для каждого ЭлементЗдание из МассивЗданийДляВыгрузки цикл
			МесяцНачислений = ПараметрыВыгрузки.НачалоПериода;
			Пока МесяцНачислений <= ПараметрыВыгрузки.КонецПериода цикл
				НоваяСтрока = ПомесячнаяТаблицаЗданий.Добавить();
				НоваяСтрока.МесяцНачислений = МесяцНачислений;
				НоваяСтрока.УникальныйИдентификатор = ЭлементЗдание;
				МесяцНачислений = ДобавитьМесяц(МесяцНачислений, 1);			
			КонецЦикла;		
		КонецЦикла;
		
		//Обработка таблицы, установка пометки об отказе выгружать начисления на дом, для выгрузки только оплат (например в текущем периоде)
		ВыборкаРазрешенныхЗданийПомесячно = ОбменМП_СлужебныйВызовСервера.ПолучитьРазрешенныеЗдания(
			ПомесячнаяТаблицаЗданий, ПараметрыВыгрузки); 
		...
		//@skip-check query-in-loop
		//Формируем таблицу начислений на все здания из плана обмена
		ПомесячнаяВыборкаНачислений = ПолучитьВыборкуНачислений(ВыборкаРазрешенныхЗданийПомесячно,
...
Функция СформироватьПакетИнформацииОСобственниках(РезультатЗапроса, ПараметрыОбмена)
	...
	Пока ЗапросВыборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(СобственникФизЛицо) И Не ПустаяСтрока(СобственникФизЛицо.ОбменМП_idMobApp) Тогда
			НомерТелефона = СокрЛП(СтрЗаменить(СобственникФизЛицо.ОбменМП_idMobApp, "-", ""));
			
			Если СтрДлина(НомерТелефона) = 12 и Лев(НомерТелефона, 3) = "+79" тогда
